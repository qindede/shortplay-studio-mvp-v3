...
class ListAssetsTests(unittest.TestCase):
    @patch("app.features.asset.db.list_assets")
    def test_project_not_found_raises(self, mock_list):
        mock_list.return_value = None
        with self.assertRaises(NotFoundError):
            asset_service.list_assets({"id": "user_1"}, "proj_missing")


class GetAssetTests(unittest.TestCase):
    @patch("app.features.asset.db.get_asset")
    def test_not_found_raises(self, mock_get):
        mock_get.return_value = None
        with self.assertRaises(NotFoundError):
            asset_service.get_asset({"id": "user_1"}, "asset_missing")


class CreateAssetTests(unittest.TestCase):
    @patch("app.features.asset.db.create_asset")
    def test_project_not_found_raises(self, mock_create):
        ...
        with self.assertRaises(NotFoundError):
            ...
            payload.name = "角色A"
            asset_service.create_asset({"id": "user_1"}, "proj_missing", payload)
    ...
    def test_insufficient_points_raises(self, mock_create):
        ...
        with self.assertRaises(InsufficientPointsError):
            ...
            payload.type = "character"
            payload.name = "角色A"
            asset_service.create_asset({"id": "user_1"}, "proj_1", payload)
...
class StartVoiceCloneTests(unittest.TestCase):
    def test_no_consent_raises(self):
        ...
        with self.assertRaises(BadRequestError):
            asset_service.start_voice_clone({"id": "user_1"}, "asset_1", payload)

    @patch("app.features.asset.service.get_asset")
...
class UpdateAssetTests(unittest.TestCase):
    @patch("app.features.asset.db.update_asset")
    def test_not_found_raises(self, mock_update):
        mock_update.return_value = None
        payload = MagicMock()
        payload.model_dump.return_value = {}
        with self.assertRaises(NotFoundError):
            asset_service.update_asset({"id": "user_1"}, "asset_missing", payload)


class DeleteAssetTests(unittest.TestCase):
    @patch("app.features.asset.db.delete_asset")
    def test_not_found_raises(self, mock_delete):
        mock_delete.return_value = False
        with self.assertRaises(NotFoundError):
            asset_service.delete_asset({"id": "user_1"}, "asset_missing")
