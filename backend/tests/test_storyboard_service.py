...
class GenerateStoryboardTests(unittest.TestCase):
    @patch("app.features.storyboard.db.episode_generation_context", new_callable=AsyncMock)
    def test_episode_not_found_raises(self, mock_ctx):
        mock_ctx.return_value = None
        with self.assertRaises(NotFoundError):
            asyncio.run(storyboard_service.generate_storyboard({"id": "user_1"}, "ep_missing"))

    @patch("app.features.storyboard.service._run_paid_generation", new_callable=AsyncMock)
    @patch("app.features.storyboard.db.save_storyboard", new_callable=AsyncMock)
    @patch("app.features.storyboard.db.episode_generation_context", new_callable=AsyncMock)
    def test_storyboard_not_found_inside_work_raises(self, mock_ctx, mock_save, mock_run):
        ...
        async def run_side_effect(user, cost, scene, desc, jtype, provider, work, **kwargs):
            return await work({"id": "job_1"})

        mock_run.side_effect = run_side_effect

        with self.assertRaises(NotFoundError):
            asyncio.run(storyboard_service.generate_storyboard({"id": "user_1"}, "ep_1"))
...
class PrepareStoryboardTests(unittest.TestCase):
    ...
    def test_episode_not_found_raises(self, mock_ctx):
        mock_ctx.return_value = None
        with self.assertRaises(NotFoundError):
            asyncio.run(storyboard_service.prepare_storyboard({"id": "user_1"}, "ep_missing"))
    ...
    def test_insufficient_points_raises(self, mock_has, mock_ctx):
        ...
        with self.assertRaises(InsufficientPointsError):
            asyncio.run(storyboard_service.prepare_storyboard({"id": "user_1"}, "ep_1"))
