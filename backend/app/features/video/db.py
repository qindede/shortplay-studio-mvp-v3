...
@require_db
async def update_job_for_video_shot(job_id: str, shot_id: str, provider_task_id: str, timestamp: datetime) -> dict[str, Any] | None:
...
@require_db
async def update_video_job_from_provider(job_id: str, remote: dict[str, Any]) -> None:
...
@require_db
async def list_video_versions(user: dict[str, Any], project_id: str, episode_id: str | None = None) -> list[dict[str, Any]] | None:
    async with AsyncSessionLocal() as db:
        ...
        rows = await db.scalars(query.order_by(VideoVersion.created_at.desc()))
        return [_video_version_dict(row) for row in rows]


@require_db
async def delete_video_version(user: dict[str, Any], version_id: str) -> bool:
    async with AsyncSessionLocal() as db:
...
@require_db
async def compose_context(user: dict[str, Any], episode_id: str) -> dict[str, Any] | None:
    from ..storyboard.db import episode_generation_context

    context = await episode_generation_context(user, episode_id)
    if not context:
        return None
    async with AsyncSessionLocal() as db:
        ...
        shots = {s.id: s for s in await db.scalars(select(Shot).where(Shot.id.in_(shot_ids)))} if shot_ids else {}
...
@require_db
async def save_composed_version(user: dict[str, Any], episode_id: str, payload: Any, video_url: str, cost: int, timestamp: str, charge: bool = True, job_id: str | None = None) -> dict[str, Any] | None:
    ...
    async with AsyncSessionLocal() as db:
        ...
        if charge:
            try:
                await change_points(db, user["id"], -cost, "consume", "合成成片", f"合成《{episode.title}》成片版本", ts)
            except ValueError as exc:
                return {"error": str(exc)}
            await add_ai_job(db, user["id"], "compose", "ffmpeg", cost, ts, episode_id=episode_id, project_id=episode.project_id)
        version_no = await db.scalar(select(func.count(VideoVersion.id)).where(VideoVersion.episode_id == episode_id)) + 1
