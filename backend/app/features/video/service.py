...
async def generate_video_for_shot(user: dict, shot_id: str) -> dict:
    from ...features.shot.service import get_generation_context as shot_generation_context
    ctx = await shot_generation_context(user, shot_id)
    ...
    async def work(job: dict) -> tuple[dict, dict]:
        ...
        result = await db.update_job_for_video_shot(job["id"], shot_id, provider_task_id, now())
        if not result:
...
async def generate_videos_for_episode(user: dict, episode_id: str) -> list[dict]:
    ...
    tasks = []
    for shot in shots:
        tasks.append(await generate_video_for_shot(user, shot["id"]))
    return tasks


async def compose_episode(user: dict, episode_id: str, payload: Any, compose_video_file: Callable[[list[dict]], str]) -> dict:
    context = await db.compose_context(user, episode_id)
    if not context:
    ...
    async def work(job: dict) -> tuple[dict, dict | None]:
        ...
        version = await db.save_composed_version(user, episode_id, payload, video_url, 0, now(), charge=False, job_id=job["id"])
        return version, None
...
async def list_video_jobs_with_poll(user: dict, episode_id: str) -> list[dict]:
    ...
    async with AsyncSessionLocal() as session:
        ...
        shots = {s.id: s for s in (await session.scalars(select(Shot).where(Shot.id.in_(shot_ids)))).all()} if shot_ids else {}
    ...
    for job in jobs:
        if job.status == "running" and job.provider_task_id:
            ...
            try:
                ...
                await db.update_video_job_from_provider(job.id, remote)
                # Re-fetch updated job
                async with AsyncSessionLocal() as session:
                    job = await session.get(AiJob, job.id)
                    if not job:
                        continue
