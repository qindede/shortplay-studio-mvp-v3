...
@require_db
async def episode_generation_context(user: dict[str, Any], episode_id: str) -> dict[str, Any] | None:
    ...
    async with AsyncSessionLocal() as db:
        ...
        shots = [_shot_dict(shot) for shot in await db.scalars(select(Shot).where(Shot.episode_id == episode_id).order_by(Shot.no))]
        return {
        ...
        try:
            for item in generated_assets:
                ...
                if duplicate:
                    continue
                if charge:
                    await change_points(db, user["id"], -asset_cost, "consume", "AI 生成素材", f"AI 生成素材《{item['name']}》", ts)
                asset = Asset(
                    id=item["id"],
                ...
                if charge:
                    await add_ai_job(db, user["id"], "image_asset", "seedream", asset_cost, ts, project_id=episode.project_id, asset_id=asset.id)

            if charge:
                await change_points(db, user["id"], -storyboard_cost, "consume", "生成分镜", f"生成/更新《{episode.title}》分镜", ts)
                await add_ai_job(db, user["id"], "storyboard", "minimax", storyboard_cost, ts, episode_id=episode_id, project_id=episode.project_id)
        except ValueError as exc:
            await db.rollback()
            return {"error": str(exc)}

        # Cancel running video jobs for this episode
        for job in await db.scalars(select(AiJob).where(AiJob.episode_id == episode_id, AiJob.type == "video_shot", AiJob.status == "running")):
            job.status = "cancelled"
            job.updated_at = ts
        await db.execute(delete(Shot).where(Shot.episode_id == episode_id))
        new_shots = []
