...
def _create_asset_common(
    ...
    if not project or not target_user:
        return None
    try:
        change_points(db, user_id, -cost, "consume", points_scene, points_desc, ts)
...
def generate_asset(user: dict[str, Any], project_id: str, payload: Any, generated_url: str | None, refs: list[dict], cost: int, provider: str, timestamp: str) -> dict[str, Any] | None:
    ...
    with SessionLocal() as db:
        ...
        if isinstance(result, dict):
            return result
        asset, asset_refs = result
        add_ai_job(db, user["id"], "image_asset" if visual_asset else "audio_asset", provider, cost, ts, project_id=project_id, asset_id=asset.id)
        db.commit()
        return _asset_dict(asset, asset_refs)
...
def update_voice_clone(user: dict[str, Any], asset_id: str, result: dict[str, Any], cost: int, timestamp: str, consume: bool = True, job_id: str | None = None) -> dict[str, Any] | None:
    ...
    with SessionLocal() as db:
        ...
        if consume:
            try:
                change_points(db, user["id"], -cost, "consume", "音色克隆", f"训练《{asset.name}》角色音色", ts)
            except ValueError as exc:
                return {"error": str(exc)}
